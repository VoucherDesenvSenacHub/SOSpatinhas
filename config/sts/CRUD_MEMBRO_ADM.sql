CREATE DEFINER=`root`@`localhost` PROCEDURE `CRUD_MEMBRO_ADM`(IN jsonData JSON)
BEGIN
	DECLARE ACAO CHAR;
    DECLARE ID_MEMBRO_ADM INT;
    ...
    SET ID_MEMBRO_ADM = CAST(JSON_UNQUOTE(JSON_EXTRACT(jsonData, '$.ID_MEMBRO_ADM')) AS UNSIGNED);
	...
	ELSEIF ACAO = 'U' THEN
		UPDATE MEMBRO_ADM SET 
			...
			TELEFONE = TELEFONE,
			SENHA = SENHA
		WHERE ID_MEMBRO_ADM = ID_MEMBRO_ADM;
	ELSEIF ACAO = 'D' THEN
		DELETE FROM MEMBRO_ADM WHERE ID_MEMBRO_ADM = ID_MEMBRO_ADM;
	ELSEIF ACAO = 'R' THEN
		SELECT * FROM MEMBRO_ADM WHERE ID_MEMBRO_ADM = ID_MEMBRO_ADM;
	ELSEIF ACAO = 'S' THEN
